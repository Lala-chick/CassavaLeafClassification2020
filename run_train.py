...
def train_one_epoch(epoch, model, loss_fn, optimizer, train_loader, device, scaler, cfg, scheduler=None, schd_batch_update=False):
    model.train()
    ...
    for step, (imgs, image_labels) in pbar:
        ...
        if mix_decision < 0.25:
            ...
            imgs, image_labels = fmix(imgs, image_labels, alpha=1., decay_power=5., shape=(cfg.img_size , cfg.img_size), device=device)
...
def main(cfg):
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        train_loader = data_loader.train_dataloader(train, cfg, trn_idx, data_root="../CassavaLeafClassification2020/data/train_images")
        valid_loader = data_loader.valid_dataloader(train, cfg, val_idx, data_root="../CassavaLeafClassification2020/data/train_images")
        
        device = torch.device(cfg_device)
        
        if is_resnet(cfg.model):
            model = models.ResNetClassifier(cfg.model, train.label.nunique(), pretrained=True).to(device)
        elif is_effnet(cfg.model):
            model = models.EffNetClassifier(cfg.model, train.label.nunique(), pretrained=True).to(device)
        else:
            model = models.ViTClassifier(cfg.model, train.label.nunique(), pretrained=True).to(device)
        scaler = GradScaler()
        ...
        for epoch in range(cfg.epoch):
            train_one_epoch(epoch, model, loss_fn, optimizer, train_loader, device, scaler, cfg, scheduler=scheduler, schd_batch_update=False)
            with torch.no_grad():
                epoch_acc = valid_one_epoch(epoch, model, loss_fn, valid_loader, device, cfg)
            if epoch_acc > best_acc:
                torch.save(model.state_dict(), save_path + "{}_fold_{}_{}.pth".format(cfg.model, fold, epoch))
                best_acc = epoch_acc
                best_epoch = epoch
